...
async function uploadImage(file: File) {
  ...
  const { error } = await supabase.storage.from("candidate-images").upload(path, file, {
    cacheControl: "3600",
    upsert: false,
...
async function buildCandidatePayload(formData: FormData): Promise<CandidatePayload> {
  const image = formData.get("image");
  const imageUrl =
    image instanceof File && image.size > 0 ? await uploadImage(image) : readString(formData, "existingImageUrl") || null;

  return {
    full_name: readString(formData, "full_name"),
    age: Number(readString(formData, "age")),
    date_of_birth: readString(formData, "date_of_birth"),
    star: readString(formData, "star"),
    ...
    additional_fields: parseAdditionalFields(readString(formData, "additional_fields")),
    image_url: imageUrl
  };
}
